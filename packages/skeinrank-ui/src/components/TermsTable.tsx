...
const columns: ColumnDef<CanonicalTerm>[] = [
  {
    ...
    cell: ({ row }) => <span className="font-medium text-slate-950">{row.original.canonical_value}</span>,
  },
  {
    accessorKey: "slot",
    header: "Slot",
  ...
  },
  {
    accessorKey: "status",
    header: "Status",
    cell: ({ row }) => <span className="text-sm capitalize text-slate-600">{row.original.status}</span>,
  },
  {
    id: "aliases",
    header: "Aliases",
      ...
      <div className="flex flex-wrap gap-1.5">
        {row.original.aliases.length > 0 ? (
          row.original.aliases.map((alias) => (
            <Badge key={alias.id} className="bg-blue-50 text-blue-700">
              {alias.alias_value}
            </Badge>
          ))
        ) : (
          <span className="text-sm text-slate-400">No aliases yet</span>
        )}
      </div>
    ),
  },
              ...
              {table.getHeaderGroups().map((headerGroup) => (
                <tr key={headerGroup.id}>
                  {headerGroup.headers.map((header) => (
                    <th key={header.id} className="border-b border-slate-200 px-5 py-3 font-semibold">
