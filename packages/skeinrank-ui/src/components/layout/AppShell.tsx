    ...
    <div className="min-h-screen bg-slate-50 text-slate-950">
      <aside className="fixed inset-y-0 left-0 hidden w-72 border-r border-slate-200 bg-white px-5 py-6 lg:block">
        <div>
          ...
          <div className="mt-1 text-sm text-slate-500">Governance Console</div>
        </div>
        ...
        <nav className="mt-8 space-y-1">
          {navigation.map((item) => (
            ...
            >
              ...
              {item.label}
              {!item.active ? <Badge className="ml-auto bg-slate-50 text-slate-400">Soon</Badge> : null}
            </button>
          ))}
        </nav>
      </aside>
        ...
        <header className="sticky top-0 z-10 border-b border-slate-200 bg-white/90 px-6 py-4 backdrop-blur">
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div>
              ...
              <p className="text-sm text-slate-500">Manage canonical terms, aliases, slots, and runtime snapshots.</p>
            </div>
